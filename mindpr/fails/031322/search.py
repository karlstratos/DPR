...
def main(args):
    ...
    from util import strtime, SimpleTokenizer, has_answer, \
        topk_retrieval_accuracy, print_performance
    ...
    # Faiss w/ GPUs seems much faster, but unfortunately each GPU stores the
    # entire index and can't be used for this large index.
    #if str(device) == 'cuda':
    ...
    for result in results:
        for candidate in result['candidates']:
            candidate['has_answer'] = has_answer(
                candidate, result['answers'], mode='string',
                tokenizer=simple_tokenizer, lower=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('passage_embs', type=str)
