...
def main(args):
    ...
    from data import DPRDataset, get_loaders, tensorize
    ...
    for epoch in range(args.epochs):
        ...
        for batch_num, batch in enumerate(loader_train if args.use_my_loader else
            ...
            if args.use_my_loader:
                random.seed(args.seed + epoch + batch_num + rank)  # Doesn't matter if data indices are properly randomized
                batch = tensorize(batch, tokenizer, args.max_length,
                                  shuffle=not args.no_shuffle)
            else:
                (samples_batch, indices), dataset = batch
                random.seed(args.seed + epoch + iterator_train.iteration)  # iteration=batch_num+1
